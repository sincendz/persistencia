...
def populate_db(session: Session = Depends(get_session)):
    ...
    # Clientes
    client1 = Client(name="João Silva", age=35, phone_number="11999999999", email="joao@email.com")
    client2 = Client(name="Maria Souza", age=28, phone_number="11888888888", email="maria@email.com")
    ...
    # Animais
    animal1 = Animal(client_id=client1.id, name="Rex", age="5", species="Cachorro", breed="Vira-lata")
    animal2 = Animal(client_id=client2.id, name="Mimi", age="3", species="Gato", breed="Siamês")
    animal3 = Animal(client_id=client1.id, name="Thor", age="2", species="Cachorro", breed="Poodle")
    ...
    # Serviços
    service1 = Service(service_name="Vacinação", type_service="PREVENCAO", price=100.0, description="Vacina V8")
    service2 = Service(service_name="Consulta", type_service="CONSULTA", price=80.0, description="Consulta geral")
    service3 = Service(service_name="Cirurgia", type_service="CIRURGIA", price=500.0, description="Cirurgia de castração")
