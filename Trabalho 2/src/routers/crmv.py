...
def search_crmv_by_status(status: str, session: Session = Depends(get_session)):
    crmvs = session.exec(select(Crmv).where(Crmv.status.ilike(f"%{status}%")))
    if not crmvs:
        raise HTTPException(
            status_code=404, detail="Nenhum crmv encontrado com esse status."
        )
    return crmvs
...
def crmvs_page(
    ...
    data = session.exec(select(Crmv).offset(offset).limit(page_size)).all()
