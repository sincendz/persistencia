...
def veterinaries_page(
    ...
    data = session.exec(select(Veterinary).offset(offset).limit(page_size)).all()
...
def open_consultations_for_a_vet(
    veterinary_id: int, session: Session = Depends(get_session)
):
    ...
    return session.exec(
        select(Consultation)
        .where(Consultation.vet_id == veterinary_id)
        .where(Consultation.data_out == None)
    ).all()


@router.post("/", response_model=Veterinary)
def create_veterinary(
    veterinary: VeterinaryBase, session: Session = Depends(get_session)
):
    ...
    if not crmv:
        ...
        raise HTTPException(
            status_code=404, detail="Crmv de veterinário passado não existe!"
        )

    if crmv_vet:
        logging.info("CRMV já atrelado a um veterinario")
        raise HTTPException(
            status_code=403, detail="CRMV já pertence a outro veterinário."
        )
...
def delete_veterinary(veterinary_id: int, session: Session = Depends(get_session)):
    ...
    if not veterinary:
        logging.error("Veterinário não encontrado")
        raise HTTPException(
            status_code=404, detail="Veretinário com esse id não existe."
        )
