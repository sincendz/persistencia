...
def search_client_by_name(client_name: str, session: Session = Depends(get_session)):
    clients = session.exec(select(Client).where(Client.name.ilike(f"%{client_name}%")))
    if not clients:
...
def client_page(
    ...
    data = session.exec(select(Client).offset(offset).limit(page_size)).all()
...
def update_client(
    client_id: int, client: ClientBase, session: Session = Depends(get_session)
):
