...
def search_animal_by_name(animal_name: str, session: Session = Depends(get_session)):
    animals = session.exec(select(Animal).where(Animal.name.ilike(f"%{animal_name}%")))
    if not animals:
...
def search_animal_by_species(species: str, session: Session = Depends(get_session)):
    animals = session.exec(select(Animal).where(Animal.species.ilike(f"%{species}%")))
    if not animals:
        logging.error(f"Animais da espécie {species} não encontrado")
        raise HTTPException(
            status_code=404, detail=f"Animais da espécie {species} não encontrado"
        )
...
def animal_page(
    ...
    data = session.exec(select(Animal).offset(offset).limit(page_size)).all()
...
def update_animal(
    animal_id: int, animal: AnimalBase, session: Session = Depends(get_session)
):
